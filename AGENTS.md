...
- Security posture: the HTTP API has **no authentication** and binds to `0.0.0.0` by design (on-prem/LAN); it must be network-isolated or fronted by an authenticating reverse proxy in production (Phase 6 roadmap). Seal IDs (`^seal-[a-f0-9]{24}$`) and SHA-512 (`^[a-f0-9]{128}$`) are the only values interpolated into vault paths and are strictly validated at the API boundary (`fraud-firewall/src/api/server.ts`) and in `verification.ts` to block path traversal / header injection; request bodies are capped at 64 MB (413). Keep these validators when adding endpoints that touch the vault.
